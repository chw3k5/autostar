...
single_part_star_names = {"hip", "hd", "hr", "ltt", "hic", "gj", "gaia", "gaia dr1", "gaia dr2",
                          ...
                          'serp', '[hhm2007]', "gss", "iso-chaii"}
multi_part_star_names = {"bd", "cod", 'cd',  "cpd", "tyc", "lp", "he", "gsc", "ucac", 'ntts', "htr", "cs",
                         "l", "ag", 'csi', "wd", 'lupus', 'elia', 'haro', "elias", 'hen'}
string_names = {"2mass", "2masx", "apm", "asas", "bas", "bps", 'cl*berkeley', 'cl*collinder', 'cl*ic4651', 'cl*melotte',
                ...
                "sst", "twa", "crbr", 'hh', "wa", 'b59', "coku", 'rx', 'scra', "sch"}
...
asterisk = {'gam', 'rho', 'ome', "tau", 'kap', 'ups', 'bet', "phi", 'omi', "psi", "eps", 'iot', "alf"}
v_asterisk = {"ab", 'ci', 'dp', 'ct', 'dh', 'uz', "yz", "nu", "mu", "rr", 'xi', 'nn', "v830", 'de', 'ny', 'ds', "hn",
              'fu', "gu", 'gq', "uz", 'hw', 'hu'}
...
gaia_dr2_parallax_offset = 0.0  # 0.03

working_dir = os.path.dirname(os.path.realpath(__file__))
working_dir, _ = working_dir.split("SpExoDisks")
working_dir = os.path.join(working_dir, "SpExoDisks", "spexodisks")
ref_dir = os.path.join(working_dir, "load", "reference_data")
spectra_dir = os.path.join(working_dir, "load", "spectral_data")
data_pro_dir = os.path.join(working_dir, "load", "data_products")
...
sb_ref_file_name = os.path.join(working_dir, "load", "reference_data", "simbad_ref_data.csv")
...
# for sorting.py
star_name_preference = ["v*", "*", "hd", 'bd', "iras", "2mass", "tyc", 'gaia dr2']
...
def rno(star_id):
    if len(star_id) > 1:
        return "RNO " + str(star_id[0]) + star_id[0]
    else:
